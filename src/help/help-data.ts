...
export const KEYWORD_HELP: Record<string, HelpEntry> = {
  ...
  nudge_px:    M('view', 'This sheet\'s separation between coincident curves, overriding page { curves { nudge_px } }. 0 draws them on top of one another.', 'nudge_px = 0;'),
  ...
  I2_min: M('faults', 'Lowest negative-sequence current for this condition. Declare I2 as well: a range without a centre is refused.', 'I2_min = 300 A;'),
  I2_max: M('faults', 'Highest negative-sequence current for this condition.', 'I2_max = 900 A;'),
  I1_min: M('faults', 'Lowest positive-sequence current for this condition.', 'I1_min = 300 A;'),
  I1_max: M('faults', 'Highest positive-sequence current for this condition.', 'I1_max = 900 A;'),
  I0_min: M('faults', 'Lowest zero-sequence current for this condition.', 'I0_min = 50 A;'),
  ...
  current_max: M('view', 'Right-hand end of the X-axis. An axis bound, paired with current_min; where a *curve* stops is I_cutoff, on the element or stage itself.', 'current_max = 50 kA;'),
  I_cutoff: M('element', 'Where this characteristic stops. Past the maximum fault the bus can deliver, the curve describes a current that cannot flow, so drawing it invites a margin to be read at a fault that does not exist -- and a grade asked for beyond it is refused rather than answered. A stage may stop earlier than the element that owns it, and falls back to the element’s.', 'I_cutoff = 8 kA;'),
  ...
  curves:      M('page', 'How the characteristics are drawn as a set: which palette, the default line weight, and how far apart coincident curves are pulled. An individual curve overrides the first two.', 'curves = { palette = "okabe_ito"; line_width_px = 2.0; };'),
  ...
  'curves.nudge_px': M('page', 'Separation drawn between curves that coincide, in pixels (default 3; 0 draws them on top of one another). Perpendicular to the line and symmetric, so the pair\'s centre is the true position, and declared in the notes.', 'nudge_px = 3;'),
