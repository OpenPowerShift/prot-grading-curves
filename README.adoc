...
a `.tc` study. This is what the playground's *Guide* button opens,
converted by Asciidoctor at build time
(`scripts/guide-plugin.js`), with a contents list and a filter.
...
The guide's own code blocks are checked against the real parser by
`tests/unit/guide.spec.ts`, so an example cannot teach a mistake.
